...
import { IonHeader, IonToolbar, IonTitle, IonContent, 
          ...
          IonNote, IonSkeletonText, IonThumbnail, IonSearchbar } from '@ionic/angular/standalone';
import { addIcons } from 'ionicons';
import { Browser } from '@capacitor/browser';
import * as allIcons from 'ionicons/icons';
import { ActivatedRoute, Router, RouterLink } from '@angular/router';
import { Preferences } from '@capacitor/preferences';
import { GetUpcomingAppointments } from '../services/get-upcoming-appointments';
...
@Component({
  ...
  imports: [CommonModule, IonHeader, IonToolbar, 
            ...
            IonSkeletonText, IonThumbnail, IonSearchbar],
  schemas: [ CUSTOM_ELEMENTS_SCHEMA ]
})
export class AppointmentsPage implements OnInit {
  appointmentsF: FormGroup;
  isLoadingApp= false;
  appointments: any[] = [];
  results: any;
  totalPayments: any;
  ...
  private markFormGroupTouched() {
    Object.keys(this.appointmentsF.controls).forEach(key => {
      this.appointmentsF.get(key)?.markAsTouched();
    });
  }

  async gettingUpcomingAppointments(){
    this.isLoadingApp = true;
    this.cdr.detectChanges();      

    try {
      const formData = this.appointmentsF.value;
      console.log('Great: ', formData);

      this.upcomingAppointments.upcomingAppointments(formData).subscribe({
        next: async (response: any) => {
          ...
          if (response) {
            if (response.success === true) {
              ...
              this.filteredAppointments = [...this.appointments]; // <-- set AFTER data arrives
